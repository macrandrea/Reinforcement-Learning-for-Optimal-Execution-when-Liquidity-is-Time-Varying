...
if __name__ == '__main__': 
    ...
    def doTrain(age, numIt = 200,inc=False):
        ...
        for j in tqdm(range(numIt)):
            ...
            for i in tqdm(range(slices)):
                ...
                loss, grad, state, epsilon, new_inv, action, reward = age.step(inv, tempo, dati, a[i]) 
                inv = new_inv 
                s_0 = dati[-1]
                tempo += 1
    ...
    def doTest(age, numIt = 100):
        ...
        for j in tqdm(range(numIt)):
            ...
            for i in tqdm(range(slices)):
                ...
                selling_strategy = []
                (new_inv, x, reward) = age.test(inv, tempo, dati, a[i])
                tempo += 1
                s_0 = dati[-1]
                inv = new_inv
                re.append(reward)
    ...
    def impl_IS_twap():
        alpha =  np.array([0.004  , 0.00361, 0.00322, 0.00283, 0.00244, 0.00205, 0.00166,  0.00127, 0.00088, 0.00049])  
        if PERM_IMP == 0.001:
            dati =   np.load('C:/Users/macri/Desktop/ennesima/dati_TWAP_tv_decrescente.npy')
        else:
            dati =   np.load('C:/Users/macri/Desktop/ennesima/dati_TWAP_tv_decrescente.npy')
    ...
    def run(n, test = False):
        ...
        age = Agente(inventario = INV, numTrain = numIt)
        act_mean, act_sd, act_hist, loss_mean, loss_sd, rew_mean, rew_sd, loss_hist, rew_hist, state, epsilon = doTrain(age, numIt, inc=True)
