...
BATCH = 16 # batch size
P_MIN = 9.95 # historical min
P_MAX = 10.05 # historical max
LAMBDA_A = 5
LAMBDA_B = 5
SIGMA_B = 0.002
SIGMA_A = 0.002    
...
class Ambiente():
    ...
    def normalise(self, inventory, time, a):
        '''
        performs the normalisation in the range [-1,1] for the feature of the NN
        '''
        q, x = self.inventory_action_transform(inventory, a)
...
class ReplayMemory():
    ...
    def add(self, inv, time, x, next_inv, next_time, reward):
...
class Agente():
    ...
    def test(self, inv, tempo, dati, alpha):
        '''
...
if __name__ == '__main__': 
    ...
    def doTrain(age, numIt = 200):
        ...
        for j in tqdm(range(numIt)):
            ...
            for i in tqdm(range(slices)):
                ...
                loss, grad, state, epsilon, new_inv, action, reward = age.step(inv, tempo, dati, a) 
                inv = new_inv 
                s_0 = dati[-1]
                tempo += 1
    ...
    def doTest(age, numIt = 100):
        ...
        for j in tqdm(range(numIt)):
            ...
            for i in tqdm(range(slices)):
                ...
                k = kappa[i][0]
                a = alpha[i][0]
                dati = Ambiente(T = 1, sigma = VOLA, S0 = s_0, action = x, kappa = k).abm(numIt = 1).flatten()
                selling_strategy = []
                (new_inv, x, reward) = age.test(inv, tempo, dati, a)
                tempo += 1
                s_0 = dati[-1]
                a_hist.append(a)
                k_hist.append(k)
        ...
        return mean_list, std_list, act, *doAve(act), *doAve(re), re, transaction_cost_balance, states, np.asarray(dat), a_hist,  k_hist
    ...
    def run(n, test = False):
        ...
        if test == True:
            ...
            ql_TW = 0#impl_IS_twap(a_hist, numTr)
            ...
            return azioni, azioni_med, sdaz, ricompensa, sdRic, re, tr, states, dat, ql_IS, ql_TW, dat
    ...
    for _ in range(1):
        ...
        azioni, azioni_med, sdaz, ricompensa, sdRic, re, tr, states, dati, ql_IS, ql_TW, dat = run(n = NUMIT, test = True)
